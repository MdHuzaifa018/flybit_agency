...
const TeamCard = ({ member, index }) => {
  ...
  return (
    <div
      className={`reveal-up delay-${index % 4}`}
      style={{
        background: hovered ? '#17150F' : 'var(--bg-elevated)',
        borderRadius: '24px',
        border: `1.5px solid ${hovered ? '#FFB800' : 'var(--border-subtle)'}`,
        overflow: 'hidden',
          ...
          {member.social?.apply ? (
            ...
            <div style={{ display: 'flex', gap: '16px' }}>
              {member.social?.linkedin && (
                <a href={member.social.linkedin} target="_blank" rel="noopener noreferrer" style={{ color: hovered ? 'rgba(255,255,255,0.7)' : 'var(--text-secondary)', transition: 'color 200ms ease' }} onMouseEnter={(e) => e.currentTarget.style.color = '#0a66c2'} onMouseLeave={(e) => e.currentTarget.style.color = hovered ? 'rgba(255,255,255,0.7)' : 'var(--text-secondary)'}>
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/></svg>
                </a>
              )}
              {member.social?.whatsapp && (
                <a href={member.social.whatsapp} target="_blank" rel="noopener noreferrer" style={{ color: hovered ? 'rgba(255,255,255,0.7)' : 'var(--text-secondary)', transition: 'color 200ms ease' }} onMouseEnter={(e) => e.currentTarget.style.color = '#25D366'} onMouseLeave={(e) => e.currentTarget.style.color = hovered ? 'rgba(255,255,255,0.7)' : 'var(--text-secondary)'}>
              ...
              {member.social?.portfolio && (
                <a href={member.social.portfolio} target="_blank" rel="noopener noreferrer" style={{ color: hovered ? 'rgba(255,255,255,0.7)' : 'var(--text-secondary)', transition: 'color 200ms ease' }} onMouseEnter={(e) => e.currentTarget.style.color = '#1A65E3'} onMouseLeave={(e) => e.currentTarget.style.color = hovered ? 'rgba(255,255,255,0.7)' : 'var(--text-secondary)'}>
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"></circle><line x1="2" y1="12" x2="22" y2="12"></line><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"></path></svg>
                </a>
              )}
              {member.social?.instagram && (
                <a href={member.social.instagram} target="_blank" rel="noopener noreferrer" style={{ color: hovered ? 'rgba(255,255,255,0.7)' : 'var(--text-secondary)', transition: 'color 200ms ease' }} onMouseEnter={(e) => e.currentTarget.style.color = '#E1306C'} onMouseLeave={(e) => e.currentTarget.style.color = hovered ? 'rgba(255,255,255,0.7)' : 'var(--text-secondary)'}>
